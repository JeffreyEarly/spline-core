...
nav_order: 4
mathjax: true
permalink: /tutorials/local-point-constraints-1d
---

# Local Point Constraints in 1D
...
caseDefinitions = struct(  "Title", {  "Value at one point",  "Zero slope at one point",  "Zero curvature at one point",  "Value and slope together",  "Zero slope at two points"},  "Constraints", {  PointConstraint.equal(0.42, value=0.55),  PointConstraint.equal(0.42, D=1, value=0),  PointConstraint.equal(0.68, D=2, value=0),  [PointConstraint.equal(0.42, value=0.55); PointConstraint.equal(0.42, D=1, value=0)],  PointConstraint.equal([0.35; 0.55], D=1, value=0)}  );
...
for iCase = 1:numel(caseDefinitions)
    ...
    for iConstraint = 1:numel(theseConstraints)
        if all(theseConstraints(iConstraint).D == 0)
            scatter(theseConstraints(iConstraint).points, theseConstraints(iConstraint).value,  55, "o", LineWidth=1.4, MarkerEdgeColor=[0.82 0.2 0.2])
        else
            for iPoint = 1:size(theseConstraints(iConstraint).points, 1)
                xline(theseConstraints(iConstraint).points(iPoint), ":",  Color=[0.82 0.2 0.2], LineWidth=1.3)
            end
        end
    end
...
![Several one-dimensional local equality constraints applied to the same noisy fit. Each panel compares the constrained fit against the unconstrained baseline.](./local-point-constraints-1d/local-constraint-cases.png)

*Several one-dimensional local equality constraints applied to the same noisy fit. Each panel compares the constrained fit against the unconstrained baseline.*
...
![A value constraint and a zero-slope constraint can be imposed at the same point by combining PointConstraint objects.](./local-point-constraints-1d/combined-local-constraints.png)

*A value constraint and a zero-slope constraint can be imposed at the same point by combining PointConstraint objects.*
