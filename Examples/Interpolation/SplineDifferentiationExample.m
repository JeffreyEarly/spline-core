...
splineFit = InterpolatingSpline(x, f(x), K=K);
...
plot(xDense, splineFit(xDense, 1), LineWidth=2)
grid on
...
plot(xDense, splineFit(xDense, 3), LineWidth=2)
plot(xDense, derivativeSpline(xDense, 2), ":", LineWidth=2)
grid on
title("Higher derivatives remain consistent")
legend("Truth", "spline(x,3)", "diff(spline)(x,2)", Location="southoutside")
